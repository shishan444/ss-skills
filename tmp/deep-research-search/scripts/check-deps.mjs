#!/usr/bin/env node
// deep-research-search 环境检查 + Chrome/Proxy 启动（v3）
// 功能：
//   1. 检查 Node 版本
...
function checkPythonDeps() {
  try {
    execSync('python3 -c "import requests,trafilatura"', {
      encoding: 'utf-8', timeout: 5000, stdio: ['pipe', 'pipe', 'pipe']
    });
    console.log('[OK] Python 依赖齐全（requests + trafilatura）');
  } catch {
    console.log('[FAIL] Python 依赖缺失（requests/trafilatura）—— cdp_client/serp_collect/cdp_fetch/url_fetch 将无法运行');
...
async function startProxy() {
  ...
  try {
    // 用 process.execPath（当前 node 绝对路径）替代裸 'node'，避免 PATH 无 node 时 ENOENT
...
async function main() {
  console.log('=== deep-research-search 环境检查 ===\n');
