#!/usr/bin/env node
// CDP Proxy - HTTP API 操控用户日常 Chrome（web-chrome skill 专用）
// 移植自 deep-research-search，TAB_POOL_LIMIT 调整为 15（web-chrome 页面池硬上限）
// 独立端口 3457（与 deep-research-search 的 3456 分离，各自独立进程，共享同一 Chrome）
...
// TabPool 硬上限：TAB_POOL_LIMIT（默认 15），防资源耗尽
...
// web-chrome 页面池硬上限：15（SKILL.md 第一档规则3）
// 默认 15，可通过环境变量 CDP_PROXY_TAB_LIMIT 覆盖（不建议）
const TAB_POOL_LIMIT = parseInt(process.env.CDP_PROXY_TAB_LIMIT || '15');
let ws = null;
let cmdId = 0;
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    // ===== CDP Input domain（isTrusted=true）=====
    // 注：background tab 的 mouseMoved 在 Chrome 中显著慢（~5s/点，触发完整合成事件管线）
    // 失效点D修正：去除每点额外 setTimeout（CDP 本身已提供节奏），并在响应中报告耗时
    // 调用方应控制点数（建议 ≤5 点），或用 fast 模式只移动端点
    else if (pathname === '/input/move') {
      const sid = await ensureSession(q.target);
      const data = JSON.parse((await readBody(req)) || '{}');
      let points = data.points || [];
      // fast 模式：只保留起点、中点、终点（3点），大幅降低延迟
      if (data.fast && points.length > 3) {
      ...
      res.end(JSON.stringify({ moved: points.length, elapsedMs: elapsed,
        warning: elapsed > 3000 ? 'mouseMoved 在 background tab 慢（Chrome 固有），建议减少点数或用 fast 模式' : null }));
    }
