"""CDP Client（防腐层 ACL）— web-chrome skill

统一封装与 cdp-proxy 的交互，隔离 proxy 协议变更对业务脚本
（tab_manager / intercept_detector）的影响。

移植自 deep-research-search，TabPool 上限调整为 15（web-chrome 页面池硬上限）。

职责：
  - 连接与健康探活（health / is_ready / wait_ready）
  - TabPool（信号量，上限默认 15；acquire_tab 返回租约，用完自动释放）
  - 原子组合操作（fetch_url：开页 → 等载 → 提取 → 关页）
...
DEFAULT_PROXY = 'http://localhost:3457'  # web-chrome 独立端口，与 deep-research-search 的 3456 分离
# web-chrome 页面池硬上限：15（与 cdp-proxy.mjs TAB_POOL_LIMIT 对齐）
DEFAULT_TAB_POOL = 15
...
class CdpClient:
    ...
    def mouse_move(self, target_id, points, fast=False):
        """points: [(x,y),...] 贝塞尔轨迹点。fast=True 时 proxy 裁剪为3点（降延迟）。

        失效点D修正：background tab 的 mouseMoved 慢（~5s/点），fast 模式避免超时。
        """
        body = {'points': points}
        if fast:
            body['fast'] = True
