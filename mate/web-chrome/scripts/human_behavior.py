"""人类行为模拟（HumanBehavior）— web-chrome skill

移植自 deep-research-search，无修改（拟人引擎逻辑与反爬实践无关，是通用能力）。
...
（后者 isTrusted=false，反爬一查即穿）。这是 web-chrome 第一档硬规则。
"""
...
class HumanBehavior:
    ...
    def move_and_click(self, target_id, x, y, w=120, h=36, fast=False):
        """人类化移动 + 点击：随机起点 → 贝塞尔曲线 → 目标随机落点 → 点击。

        失效点D修正：background tab 的 mouseMoved 慢（~5s/点），默认用 fast 模式
        （起点+中点+终点3点），完整贝塞尔曲线（10-20点）仅在 fast=False 时用。
        fast 模式仍是曲线（3点贝塞尔），保留 isTrusted=true，只是点数少。
        """
        ...
        # fast 模式：3点（起/中/终），完整模式：10-20点
        n = 3 if fast else random.randint(10, 20)
        pts = bezier_points((sx, sy), c1, c2, (tx, ty), n=n)
        # mouse_move 内部走 proxy /input/move；fast 时传 fast 标志让 proxy 再裁剪
    ...
    def click_selector(self, target_id, selector, fast=True):
        """对 CSS 选择器元素做人类点击：先 eval 取包围盒，再 move_and_click。
        返回 (success, error)。
        fast 默认 True（3点贝塞尔），因 background tab mouseMoved 慢（失效点D）。"""
        ...
        self.move_and_click(target_id, box['x'], box['y'], box['w'], box['h'], fast=fast)
        return True, None
